...
OkoljeIzvedba <- 'C:/Users/Podlogar/Documents/DES/biopsy'
...
## podatki
data(biopsy)
osnovniPodatki <- na.omit(biopsy)
summary(osnovniPodatki)
onsovniPodatki_response <- osnovniPodatki$class
onsovniPodatki_FM <- osnovniPodatki[, -c(1,11)]
...
matrikaAcc <- data.frame(matrix(NA, length(baseLearner),4))
rownames(matrikaAcc) <- baseLearner
colnames(matrikaAcc) <- c('CV1','CV2','CV3','CV4')
vect <- 1:length(names(cvRazdelitev))
for(i in 1:length(vect)){
	OkoljeRezultat <- paste0(OkoljeIzvedba, '/Rezultati')
	OkoljeRezultatCV <- paste0(OkoljeRezultat, '/CV', i)
	...
	for(bl in baseLearner){
		matrikaAcc[bl, i] <- confusionMatrix(table(napovedClass[,bl],yValid))[[3]]['Accuracy']
	}
}
